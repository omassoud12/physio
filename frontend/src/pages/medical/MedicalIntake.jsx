...
function Field({ label, ar, required, error, children, className = '' }) {
  const errorId = useId()
  const control = error && isValidElement(children) ? cloneElement(children,{'aria-describedby':errorId}) : children
  return <label className={`medical-field ${error ? 'medical-field--invalid' : ''} ${className}`}><BilingualLabel fr={label} ar={ar} required={required}/>{control && <span className="medical-control">{control}</span>}{error && <span className="medical-error" id={errorId} role="alert">{error}</span>}</label>
}

function TextInput({ value, onChange, type='text', min, max, step, disabled, required, error, ...labels }) {
  return <Field {...labels} required={required} error={error}><input type={type} value={value ?? ''} min={min} max={max} step={step} disabled={disabled} required={required} aria-invalid={Boolean(error)} onChange={(e)=>onChange(e.target.value)} /></Field>
}

function RadioGroup({ label, ar, value, onChange, options, required=false, error }) {
  const groupName = useId()
  const errorId = useId()
  return <fieldset className={`medical-field medical-fieldset ${error ? 'medical-field--invalid' : ''}`} aria-describedby={error ? errorId : undefined}><legend><BilingualLabel fr={label} ar={ar} required={required}/></legend><div className="medical-options">{options.map(([key,fr,a])=><label className="medical-choice" key={key}><input type="radio" name={groupName} checked={value===key} required={required} aria-invalid={Boolean(error)} onChange={()=>onChange(key)} /><span><span dir="ltr">{fr}</span><span dir="rtl" lang="ar">{a}</span></span></label>)}</div>{error && <span className="medical-error" id={errorId} role="alert">{error}</span>}</fieldset>
}
...
function SectionTitle({ fr, ar, note, number }) {
  const sectionNumber = number ?? {'Medical documents':4,'Screening: warning signs':5,'Review and confirmation':7}[fr]
  return <div className="medical-section-title"><span className="medical-section-index">{String(sectionNumber).padStart(2,'0')} / {steps.length}</span><h2 dir="ltr">{fr}</h2><h3 dir="rtl" lang="ar">{ar}</h3>{note && <p>{note}</p>}</div>
}
...
export default function MedicalIntake() {
  ...
  useEffect(()=>{
    ...
    const firstInvalid = medicalFormRef.current?.querySelector('[aria-invalid="true"]')
  ...
  function validateSubmit(){
    const personalRequiredErrors = {}
    if(!personal.first_name.trim()) personalRequiredErrors.first_name='Complete this required field / يرجى إكمال هذا الحقل المطلوب'
    if(!personal.last_name.trim()) personalRequiredErrors.last_name='Complete this required field / يرجى إكمال هذا الحقل المطلوب'
    if(!personal.date_of_birth) personalRequiredErrors.date_of_birth='Complete this required field / يرجى إكمال هذا الحقل المطلوب'
    if(!personal.sex) personalRequiredErrors.sex='Choose an option / يرجى اختيار أحد الخيارات'
    if(Object.keys(personalRequiredErrors).length){setValidationErrors(personalRequiredErrors);setStep(0);setMessage({error:true,text:'Complete the required personal fields / يرجى إكمال الحقول الشخصية المطلوبة'});return false}
    const personalValueErrors = {}
    if(new Date(`${personal.date_of_birth}T00:00:00`)>new Date()) personalValueErrors.date_of_birth='Check the date of birth / يرجى التحقق من تاريخ الميلاد'
    if(!bmiFrom(personal.height_cm,personal.weight_kg)) {
      personalValueErrors.height_cm='Check the height and weight / يرجى التحقق من الطول والوزن'
      personalValueErrors.weight_kg='Check the height and weight / يرجى التحقق من الطول والوزن'
    }
    if(Object.keys(personalValueErrors).length){setValidationErrors(personalValueErrors);setStep(0);setMessage({error:true,text:'Check the date of birth, height, and weight / يرجى التحقق من تاريخ الميلاد والطول والوزن'});return false}
    if(!subjective.episode_duration){setValidationErrors({episode_duration:'Choose a duration / يرجى اختيار المدة'});setStep(5);setMessage({error:true,text:'Choose how long the pain usually lasts / يرجى اختيار مدة الألم المعتادة'});return false}
    setValidationErrors({})
    return true
  }
  ...
  return <DashboardLayout role="Patient / مريض" title="Medical record / الملف الطبي" subtitle="Medical form and physiotherapy assessment / الاستمارة الطبية وتقييم العلاج الفيزيائي">
    <div className="medical-shell">
      ...
      <main className="panel medical-form-card" ref={medicalFormRef}>
        {step===0&&<section><SectionTitle number={1} fr="Personal information" ar="المعلومات الشخصية" note="Fields marked with an asterisk are required. / الحقول المعلّمة بنجمة مطلوبة."/><div className="medical-grid">
          <TextInput label="Last name" ar="اسم العائلة" required error={validationErrors.last_name} value={personal.last_name} onChange={(v)=>updatePersonal('last_name',v)}/><TextInput label="First name" ar="الاسم الأول" required error={validationErrors.first_name} value={personal.first_name} onChange={(v)=>updatePersonal('first_name',v)}/>
          <TextInput label="Date of birth" ar="تاريخ الولادة" required error={validationErrors.date_of_birth} type="date" max={new Date().toISOString().slice(0,10)} value={personal.date_of_birth} onChange={(v)=>updatePersonal('date_of_birth',v)}/><TextInput label="Age — calculated automatically" ar="العمر — يُحسب تلقائياً" value={personal.age} disabled onChange={()=>{}}/>
          <RadioGroup label="Sex" ar="الجنس" required error={validationErrors.sex} value={personal.sex} onChange={(v)=>updatePersonal('sex',v)} options={[["female","Female","أنثى"],["male","Male","ذكر"],["other","Other / prefer not to say","آخر / أفضل عدم التحديد"]]}/><RadioGroup label="Marital status" ar="الحالة الاجتماعية" value={personal.marital_status} onChange={(v)=>updatePersonal('marital_status',v)} options={[["single","Single","أعزب/عزباء"],["married","Married","متزوج/ة"],["other","Other","أخرى"]]}/>
          <div className="medical-related-fields"><TextInput label="Height in cm" ar="الطول بالسنتيمتر" required error={validationErrors.height_cm} type="number" min="80" max="250" value={personal.height_cm} onChange={(v)=>updatePersonal('height_cm',v)}/><TextInput label="Weight in kg" ar="الوزن بالكيلوغرام" required error={validationErrors.weight_kg} type="number" min="20" max="350" step="0.1" value={personal.weight_kg} onChange={(v)=>updatePersonal('weight_kg',v)}/></div>
        ...
        {step===1&&<section><SectionTitle number={2} fr="Medical history" ar="التاريخ الطبي"/><div className="medical-grid">
        ...
        {step===2&&<section><SectionTitle number={3} fr="Risk factors" ar="عوامل الخطورة"/><div className="medical-grid">
        ...
        {step===3&&<section><SectionTitle fr="Medical documents" ar="المستندات الطبية" note="حمّل المستندات الممسوحة ضوئياً، التقارير الطبية، صور الأشعة، صور الرنين المغناطيسي، الوصفات الطبية، أو أي ملفات طبية أخرى."/><form className="medical-upload" onSubmit={uploadDocument}><Field label="Category" ar="فئة المستند"><select value={docDraft.category} required onChange={(e)=>setDocDraft({...docDraft,category:e.target.value})}><option value="">Select / اختر</option>{documentCategories.map(([key,fr,ar])=><option value={key} key={key}>{fr} / {ar}</option>)}</select></Field><Field label="Optional description" ar="وصف اختياري"><input value={docDraft.description} onChange={(e)=>setDocDraft({...docDraft,description:e.target.value})}/></Field><Field label="File" ar="الملف"><input type="file" required accept=".pdf,.jpg,.jpeg,.png,.webp,application/pdf,image/jpeg,image/png,image/webp" onChange={(e)=>setDocDraft({...docDraft,file:e.target.files?.[0] || null})}/></Field><button className="button" disabled={uploading}>{uploading?`Uploading ${uploadProgress}% / جارٍ الرفع`:'Upload / رفع المستند'}</button></form><div className="medical-documents">{documents.map((doc)=><article key={doc.id}><div><strong>{doc.original_filename}</strong><span>{documentCategories.find(([key])=>key===doc.category)?.slice(1).join(' / ')} · {(doc.file_size/1024/1024).toFixed(2)} MB</span></div><div><button className="text-button" type="button" onClick={()=>openDocument(doc.id)}>View / عرض</button><button className="text-button medical-remove" type="button" onClick={()=>removeDocument(doc.id)}>Delete / حذف</button></div></article>)}{!documents.length&&<p className="medical-empty">No documents / لا توجد مستندات</p>}</div></section>}
        ...
        {step===5&&<section><SectionTitle number={6} fr="Subjective assessment: pain" ar="التقييم الذاتي: الألم" note="Tell us how the pain started, feels, behaves, and affects your life. / أخبرنا كيف بدأ الألم، وما طبيعته، وكيف يتغير، ومدى تأثيره في حياتك."/><div className="medical-grid">
          ...
          <RadioGroup label="Does the pain feel superficial or deep?" ar="هل يبدو الألم سطحياً أم عميقاً؟" value={subjective.pain_depth} onChange={(v)=>updateSubjective('pain_depth',v)} options={[["superficial","Superficial","سطحي"],["deep","Deep","عميق"],["both","Both","كلاهما"],["unsure","Not sure","غير متأكد"]]}/><BodyChart label="Mark every painful area on the body chart" ar="حدّد جميع مناطق الألم على مخطط الجسم" values={subjective.pain_locations} primaryArea={subjective.primary_pain_location} painSide={subjective.pain_side} onChange={(v)=>updateSubjective('pain_locations',v)}/>
        ...
        {step===6&&<section><SectionTitle fr="Review and confirmation" ar="المراجعة والتأكيد"/>{positiveFlags.length>0&&<div className="medical-emergency"><strong>{positiveFlags.length} positive warning sign(s) / {positiveFlags.length} علامة/علامات تحذيرية إيجابية</strong></div>}<label className="medical-confirm"><input type="checkbox" required/><span><span dir="ltr">I confirm that this information is accurate to the best of my knowledge.</span><span dir="rtl" lang="ar">أؤكد أن هذه المعلومات صحيحة حسب معرفتي.</span></span></label><button className="button medical-submit" type="button" disabled={saving} onClick={submitFinal}>{saving?'Submitting… / جارٍ الإرسال…':'Submit record / إرسال الملف'}</button></section>}
        <footer className="medical-actions"><button type="button" className="text-button" disabled={step===0} onClick={()=>setStep((v)=>v-1)}>← Previous / السابق</button><div className="medical-actions__forward"><button type="button" className="button button--secondary" disabled={saving} onClick={()=>save(false)}>{saving?'Saving… / جارٍ الحفظ…':'Save draft / حفظ المسودة'}</button>{step<6?<button type="button" className="button" onClick={()=>setStep((v)=>v+1)}>Next / التالي →</button>:<button className="button" type="button" disabled={saving} onClick={submitFinal}>{saving?'Submitting… / جارٍ الإرسال…':'Submit record / إرسال الملف'}</button>}</div></footer>
      </main>
    </div>
  </DashboardLayout>
}
